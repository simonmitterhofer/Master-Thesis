...
run_calibration <- function(results, scenario_analysis) {
  
  cat("   - Downloading market data...\n")
  market_data <- download_sp500_market_data(start_date ="2023-02-15", end_date = "2023-02-15")
...
process_market_data <- function(options_data, config) {
  ...
  options_data_clean <- options_data_clean %>%
    mutate(
      log_moneyness_bin = cut(log_moneyness, breaks = seq(-2, 2, by=0.02)),
  ...
  colnames(iv_matrix_market) <- paste0("K", round(exp(iv_interp$x) * S0))
